		...
		this.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				...
				if(option != 1) {
	                ...
	                if(option == 2) rubbers.add(new Rubber(drawStart));
	                drawEnd = drawStart;
	                if(option == 3) rectangle = new RectangleOrCircle(drawStart, drawEnd);
	                if(option == 4) line = new Line(drawStart, drawEnd);
	                if(option == 5) circle = new RectangleOrCircle(drawStart, drawEnd);
				}
...
//			    lineColors.add(lineColor);
		...
		addMouseMotionListener(new MouseAdapter() {
			public void mouseDragged(MouseEvent e) {
				...
				drawEnd = new Point(e.getX(), e.getY());
				if(option == 2) rubbers.add(new Rubber(drawEnd));
				if(option == 3) {
					rectangle.getPoint(drawEnd);
					rectangles.add(rectangle);
				}
...
//        g2d.setColor(lineColor);
        ...
        for(Color color : lineColors) g2d.setColor(color);
	    g2d.setStroke(bs1);
	    for(Pencil pencil : pencils) {
	        	pencil.drawPencil(g);
	    }
		for(RectangleOrCircle rec : rectangles) rec.drawRectangles(g);
		for(Line line : lines) line.drawLines(g);
	    for(RectangleOrCircle circle : circles) circle.drawCircle(g);
	...
	public void getNumber (int option) {
		this.option = option;
	}
	
	public void getColor (Color lineColor) {
		this.lineColor = lineColor;
	}

	public void getWidth (int lineWidth) {
		this.lineWidth = lineWidth;
	}
}
